...
# Will hold the pipenv command (possibly via py -X.Y -m pipenv).
# Note: under `set -u`, expanding an empty array is an error.
# We always expand it as "${PIPENV_CMD[@]:-}" to be safe.
...
find_python_path() {
  local ver="${1:?Missing python version}"
  if command -v py >/dev/null 2>&1; then
...
python_version_tuple() {
  local py_cmd="${1:?Missing python executable}"
  "$py_cmd" - <<'PY'
import sys
...
is_supported_combo() {
  local py_major="${1:?Missing py_major}"
  local py_minor="${2:?Missing py_minor}"
  local spec="${3:?Missing django spec}"
...
install_with_retry() {
  local pipfile="${1:?Missing pipfile}"
  local py_path="${2:?Missing python path}"
  local max_attempts=3
  local attempt=1

  while (( attempt <= max_attempts )); do
    if PIPENV_PIPFILE="$pipfile" PIPENV_VENV_IN_PROJECT=1 \
      "${PIPENV_CMD[@]:-}" --python "$py_path" install --deploy --skip-lock >/dev/null; then
      return 0
    fi
...
run_combo() {
  local py_ver="${1:?Missing python version}"
  local django_spec="${2:?Missing django spec}"
...
EOF
  ...
  PIPENV_PIPFILE="$tmpdir/Pipfile" PIPENV_VENV_IN_PROJECT=1 \
    "${PIPENV_CMD[@]:-}" run python "$ROOT_DIR/manage.py" test
